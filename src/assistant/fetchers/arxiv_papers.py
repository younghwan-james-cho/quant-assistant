...
def latest_arxiv_qfin(limit: int = 10) -> List[PaperItem]:
    """Fetch latest q-fin papers via arXiv RSS using HttpClient."""
    client = HttpClient()
    try:
        r = client.get(ARXIV_QFIN_FEED)
        feed = feedparser.parse(r.text)
    except Exception:
        feed = feedparser.parse(ARXIV_QFIN_FEED)  # Fallback
    ...
    for entry in feed.entries[:limit]:
        ...
        get = (
            ...
            else lambda k, default=None: getattr(entry, k, default)
        )
        ...
        authors = [
            a.get("name") if hasattr(a, "get") else getattr(a, "name", None) for a in authors_raw
        ]
